...
public class Game implements GameState
{
    ...
    protected Canvas      m_canvas;         // the display area for the game

    private int         m_view_top_clip;         // Screen area above viewport
    private int         m_view_bottom_clip;      // Screen area below viewport
    ...
    private void init(ResourceManager rm, Canvas canvas)
    {
    ...
    public Game(ResourceManager rm, Canvas canvas, byte mode) throws Exception
    {
        init(rm, canvas);
        if (mode == NORMAL)
        {
    ...
    public void addObject(/*@ nullable @*/GameObject go)
    {
    ...
    void handleScroll(int x, int y, int delta)
    {
            if (m_is_lookahead)
            {
                int max = Common.mul(Common.toFP(m_canvas.getWidth()), 200);
            ...
            m_scroll_x = Common.toInt(x) - m_canvas.getHeight() / 2 + Common.toInt(m_scroll_offset_x);
            m_scroll_y = Common.toInt(y) - getViewHeight() / 2 + 20 - getViewStartHeight();
    ...
    void limitScroll()
    {
            int h = getViewHeight() + getViewStartHeight();
            ...
            else if (m_scroll_y < m_bound_top + getViewStartHeight()) m_scroll_y = m_bound_top + getViewStartHeight();
            if (m_scroll_x < m_bound_left) m_scroll_x = m_bound_left;
            else if (m_scroll_x > m_bound_right - m_canvas.getWidth()) m_scroll_x = m_bound_right - m_canvas.getWidth();
    ...
    public void handle(int delta) throws Exception
    {
        ...
        switch(m_game_handler.getScrollType())
        {
            ...
            case GameHandler.OBJECT:
            {
                ...
                if (go instanceof Rotatable)
                {
                    ...
                    m_scroll_offset_x += Common.mul(Common.mul(Common.toFP(m_canvas.getWidth())/2, Common.cos(angle)), delta) ;
     ...
     public void draw(Graphics g)
     {
        ...
        m_terrain.drawTerrain(g, m_scroll_x, m_scroll_y, m_canvas.getWidth(), getViewHeight());
        ...
        g.setClip(0, 0, m_canvas.getWidth(), m_canvas.getHeight());
...
// ==========================================================================;
     ...
     public Drawable getPlaneDrawable(int index)
    {
     ...
     {
         m_view_top_clip = m_view_bottom_clip = 0;
     }

     public void setScreenOptions(int topClipHeight, int bottomClipHeight)
     {
         m_view_top_clip = topClipHeight;
         m_view_bottom_clip = bottomClipHeight;
     }

     protected int getViewHeight()
     {
         return m_canvas.getHeight() - m_view_top_clip - m_view_bottom_clip;
     }

     protected int getViewStartHeight()
     {
         return m_view_top_clip;
     }

     public void resetClip(Graphics g)
     {
         g.setClip(0, getViewStartHeight(), m_canvas.getWidth(), getViewHeight());
     }
    ...
    public void clearSpecialState(byte type)
     ...
     public boolean isTarget(/*@ nullable @*/GameObject go)
     {
