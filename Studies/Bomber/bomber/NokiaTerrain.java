...
public class NokiaTerrain implements Terrain
{
     ...
     protected int[]          m_pixels;             // stores pixels for temporary drawing
     ...
     protected int[]          m_sky_color;          // color of the sky at specific altitude
     protected ResourceManager
                              m_resource_manager;


    protected final static int[] c_terrain_table =
    {
           // RRGGBBAA
           0xFF000000,
           0xFF001100,
    ...
    protected final static int[] c_grass_table =
    {
           // RRGGBBAA
           0xFF118833,
           0xFF118822,
...
// ==========================================================================;
     ...
     public NokiaTerrain(GameState game_state, byte[] data, short image_height, short width, short height, ResourceManager resource_manager, boolean is_background)
    {
        ...
        m_pixels = new int[m_image_width * m_image_height];
        ...
        m_sky_color = new int[m_image_height];
...
protected void calculateSkyColor(Graphics g)
{
    for (int i = 0; i < m_sky_color.length + m_water_level; i++)
    {
        if (m_is_background)
        {
            m_sky_color[i] = 0xFFBBBBBB;
        }
        else
        {
            m_sky_color[i] = 0xFF00BBEE;
        }
    }
    for (int i = m_sky_color.length + m_water_level; i < m_sky_color.length; i++)
    {
        m_sky_color[i] = 0xFF0088FF;
    }
}
...
// ==========================================================================;
    ...
    private void drawSky(Graphics g)
    {
        ...
        {
            g.drawRGB(m_sky_color, 0, 1, i, 0, 1, m_image_height, false);
        }
...
// ==========================================================================;
    ...
    private void drawTerrainImage(Graphics g, int sx)
    {
	  ...
	  for (int x = 0; x < m_image_width; x++)
          {
                // If drawing is past the end of ground data, draw sky and water.
                if (sx + x >= m_data.length)
                {
                    index = x;
                    // for height between ceiling and water
                    for (int y = 0; y < m_image_height + m_water_level; y++)
                    {
                        // Store pixels in a vertical strip in m_pixels
                        m_pixels[index] = m_sky_color[y];
                        index += m_image_width;
                    }
                    // for height below water
                    for (int y = 0; y < - m_water_level; y++)
                    {
                        // Store pixels in a vertical strip in m_pixels
                        m_pixels[index] = 0xFF0088FF;
                        index += m_image_width;
                    }
                ...
                // Otherwise, calculate ground level for this x
                else
                {
                    // Note: current_ground is actually the amount of sky
                    current_ground = m_image_height - m_data[sx + x];
                }

                index = x;
                // for height above ground, draw sky
                ...
                // If water level is above ground level, draw water
                if (-m_data[sx + x] > m_water_level)
                {
                            int i = m_image_height + m_water_level;
                            // Draw water until ground or bottom of image is reached
                            for (index = x + m_image_width * i; i < Math.min(current_ground, m_image_height); i++)
                            {
                                // Store pixels in a vertical strip in m_pixels
                                m_pixels[index] = 0xFF0088FF;
                                index += m_image_width;
          ...
          g.drawRGB(m_pixels, 0, m_image_width, 0, 0, m_image_width, m_image_height, false);
    }
