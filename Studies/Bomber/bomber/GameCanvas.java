...
public class GameCanvas extends Canvas implements GameScreen, Runnable
{
    ...
    public void paint(Graphics g)
    {
        ...
        if (m_paused)
        {
            ...
            Helper.drawStringCenter(g, Str.paused, getHeight() / 2, getWidth(), ResourceManager.FONT_YELLOW);
            if (m_game_mode != Game.INTRO)
            {

                m_resource_manager.getBomberTitle().drawImage(g, 4, 0, getHeight() - 30);   // draw empty banner

                Helper.drawString(g, Str.exit, 2, getHeight() - 12,  ResourceManager.FONT_RED);
                Helper.drawStringLeft(g, Str.resume, getWidth(), getHeight() - 12,  ResourceManager.FONT_GREEN);
            }
        }
    ...
    protected synchronized void keyPressed(int keyCode)
    {
        ...
        if (gameAction == GAME_A || gameAction == GAME_B)
        {
            if (m_game_mode == Game.INTRO)
            {
                m_paused = false;
  ...
  //              else if (keyCode == Canvas.KEY_NUM1 || keyCode == Canvas.KEY_POUND || keyCode == Canvas.KEY_STAR) m_game.event(GameHandler.FIRE_B, true);
    ...
    public synchronized void start() throws Exception
    {
        if (m_thread == null)
        {
            ...
            m_game = new Game(m_resource_manager, this, m_game_mode);
