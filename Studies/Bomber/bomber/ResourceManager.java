...
public class ResourceManager
{
    ...
    private Player loadSound(DataInputStream dis) throws IOException, MediaException {
        int len = dis.readInt();        // sound length
        byte[] data = new byte[len];    // load to buffer
        dis.read(data, 0, len);         // read buffer
        Player ret_val = Manager.createPlayer(new java.io.ByteArrayInputStream(data), "audio/x-wav");
        ret_val.setLoopCount(1);
        ret_val.realize();
        return ret_val;
    }
    ...
    public synchronized void sound(byte type) {
        if (m_storage.getSoundEnabled() == false)
            return;
        int index = -1;
        ...
        if (index != -1) {
            try {
                for (int i = 0; i < 3; i++) {
                    if (m_sounds[i].getState() != Player.REALIZED)
    ...
    private void loadBitmapFont(DataInputStream dis) throws IOException
    {
            ...
            //
            m_bitmap_font= new BitmapFont(bitmap_image, c_start, c_end, start_pos, sub_fonts_height);       // WARN ME:
    }
